...
func (r *SafeRunner) ValidateScript(ctx context.Context, command string, workingDir string) error {
	...
	// mvdan.cc/sh handles UnsetParameterError internally: it prints to stderr
	// and returns a generic "exit status 1". Capture stderr to recover the name.
	var stderrBuf strings.Builder

	interpRunner, err := interp.New(
		// nounset: turn `$UNSET/foo` into an explicit error instead of `/foo`.
		interp.Params("-u"),
		interp.CallHandler(callFunc),
	...
	if err := interpRunner.Run(ctx, prog); err != nil {
		if name, ok := extractUnboundVarName(stderrBuf.String()); ok {
			return fmt.Errorf(
				"unbound variable %q: not set in the validator's environment. "+
					"Set %s, or use ${%s:-default} in the script",
				name, name, name,
			)
		}
		return err
...
// extractUnboundVarName scans for "<NAME>: unbound variable" lines that
// mvdan.cc/sh emits when nounset triggers.
